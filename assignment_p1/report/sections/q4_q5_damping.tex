...
\paragraph{Air damping.} The top-side area of the moving structure is
\begin{equation}
  A_{m}=L W_{m} + L_{m} W_{m} + N L_{c} W_{c} = \SI{5.00e-9}{\meter\squared},
\end{equation}
giving an air damping coefficient
\begin{equation}
  b_{\mathrm{air}}=c_{\mathrm{air}}\,A_{m}=\SI{8.08e-8}{\newton\second\per\meter},
  \qquad
  Q_{\mathrm{air}}=\frac{\sqrt{km}}{b_{\mathrm{air}}}=120.
\end{equation}
...
\begin{equation}
  b=b_{\mathrm{air}}+b_{\mathrm{clamp}}=\SI{1.00e-7}{\newton\second\per\meter},
  \qquad
  \frac{1}{Q}=\frac{1}{Q_{\mathrm{air}}}+\frac{1}{Q_{\mathrm{clamping}}}
  \;\Rightarrow\; Q\approx 96.8.
\end{equation}
Air damping dominates ($Q_{\mathrm{air}}\!=\!120 \ll Q_{\mathrm{clamping}}\!=\!500$),
so $Q$ is essentially set by the surrounding gas.

\paragraph{Settling time.} With $\zeta = b/(2\sqrt{km})=1/(2Q)\approx 5.2\times10^{-3}$
...
roughly $t_{s}\approx \SI{139}{\micro\second}$.}
\label{fig:step}
\end{figure}
...
\begin{equation}
  A_{\max,\mathrm{DC}} \;=\; \frac{g_{0} k}{m} \;=\; g_{0}\,\omega_{0}^{2}
   \;\approx\; \SI{3.46e6}{\meter\per\second\squared}
   \;\approx\; \SI{3.5e5}{\textit{g}}.
\end{equation}
...
\begin{equation}
  A_{\max,\mathrm{res}} \;=\; \frac{g_{0}\,\omega_{0}\,b}{m}
                          ...
                          \;\approx\; \SI{3.6e4}{\meter\per\second\squared}
                          \;\approx\; \SI{3.6e3}{\textit{g}}.
\end{equation}
Resonance reduces shock tolerance by a factor $Q$. The DC value
($\sim\!10^{5}\,g$) is enormous, but at $f_{0}$ the device is roughly $100\times$
more vulnerable. Fig.~\ref{fig:amax} plots $A_{\max}(f)$.
...
$f_{0}\approx \SI{662}{\kilo\hertz}$, where the device is most sensitive to
