...
\textcolor{blue}{Shortening the rise well below the \SI{2}{\micro\second}
period recovers the $\approx\!2\times$ overshoot of an ideal step, confirming
the difference is purely input filtering and not the electromechanics.}
...
-- roughly half of what $Q\approx83$ allows -- which is unphysical: at
\SI{1.6}{\pico\meter} it sits $2500\times$ below the well-resolved
nanometre-scale \SI{50}{\volt} trace, so the generalised-$\alpha$ integrator's
\emph{numerical} dissipation, not the Rayleigh damping, governed its apparent
decay. Resolving it directly is the reviewer's relative-tolerance-$10^{-7}$
route, helped by reduced algorithmic damping ($\rho_\infty\!\to\!1$) and a
manual picometre-scale tolerance on the displacement. The physical answer,
however, follows from linearity alone: far below pull-in the \SI{1}{\volt}
transient is exactly the \SI{50}{\volt} trace scaled by $1/2500$, so it settles
in the resolved \SI{50}{\volt} time, \SI{152}{\micro\second} -- the same as Q3
-- not the \SI{75}{\micro\second} of the under-resolved run. The corrected
\SI{1}{\volt} settling time is therefore \SI{152}{\micro\second}.}
